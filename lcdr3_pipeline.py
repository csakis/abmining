...
for rec in SeqIO.parse(fastq_file, 'fastq'):
    ...
    if keep_seq:
        ...
        good_reads[seq_name] = trimmed_seq

#write good sequences into file
for seq_name, dna_seq in good_reads.items():
    f_out.write('%s, %s\n' % (seq_name, dna_seq))
...
"""Searching for CDR3s."""
cdr_file_name = sample_name + '.lcdr3'  #The file name that will contain the HCDR3 sequences.
cdr_dna_file_name = sample_name + '.dna.lcdr3'  #The file name that will contain the HCDR3 DNA sequences.

# *** Here is the regular expression pattern. ***
cdr3_pattern = r'([TC][A][TC])([TC][AT][TC])([TC]G[TC])([ATCG]{3}){5,13}(T[CT][TC])(G[AG][AGCT]{4})((GG|AC)[AGCT])'  #LCDR3 search pattern
if not os.path.isfile(trim_fasta_file_name):
    sys.exit('Please check that you have the trimmed fasta file in the directory')
stars()
print 'Now we are looking for LCDR3 sequences'
seq_count = 0
cdr3_count = 0
...
for line in fasta_list:  #Go through each line of the trimmed fasta file
    ...
    if match:
        cdr3_count += 1
        cdr3_peptide =  protein_translate(aa_dict, match.group()[6:-3])
        cdr_file_out.write('%s, %s\n' % (seq_name, cdr3_peptide))  #write the CDR3 output file
        cdr_dna_file_out.write('%s, %s, %s\n' %(seq_name, cdr3_peptide, match.group()[2:-1]))  #write the CDR3 DNA output file

    else:
        rev_match = re.search(cdr3_pattern, DNA_seq)  #check if the seq contains a CDR3
        if rev_match:
            cdr3_count += 1
            cdr3_peptide =  protein_translate(aa_dict, rev_match.group()[6:-3])
            cdr_file_out.write('%s, %s\n' % (seq_name, cdr3_peptide))  #write the CDR3 output file
            cdr_dna_file_out.write('%s, %s, %s\n' %(seq_name, cdr3_peptide, rev_match.group()[2:-1]))  #write the CDR3 DNA output file
    if (seq_count % 250000) == 0:
        print 'So far %d sequences have been checked.' % seq_count
#End CDR3 search loop
stars()
print '%d LCDR3s were found.' % cdr3_count
fasta_file.close()  #close trimmed fasta file
cdr_file_out.close()  #close cdr file
cdr_dna_file_out.close()
print 'The %s file has been created and contains all the LCDR3s.' % cdr_file_name
stars()
# End of CDR3 search

# CDR3 clustering begins here
print 'Now we will find each unique LCDR3s and their abundance.'
bins_out = 'bins_' + sample_name + '.lcdr3.csv'
cdr_in = open(cdr_file_name, 'rU')
cdr_list = cdr_in.readlines()
stars()
print 'There are %d LCDR3s to search.' % (len(cdr_list))
cdr3_count = 0
cdr3_dict = {}
for line in cdr_list:  #read CDR3s line by line
    ...
    if not cdr3_count % 100000:
        print '%d of LCDR3s have been searched so far.' % cdr3_count
cdr_in.close()
stars()
print 'There are %d unique LCDR3s found' % (len(cdr3_dict))
no_of_unique_cdr3 = len(cdr3_dict)
f_out = open(bins_out, 'w')
cdr3_count = 0
print 'Here are the top 20 LCDR3s with their abundance'
stars()
print 'Frequency\t\t\tLCDR3 Sequence\n'
...
f_out.close()
stars()
print'The %s file contains the unique LCDR3s and their abundance in a csv format' % bins_out
#CDR3 clustering ends

# CDR3 DNA  clustering begins here
print 'Now we will find each unique LCDR3 DNA sequences and their abundance.'
bins_dna_out = 'bins_' + sample_name + '.dna.lcdr3.csv'
...
for line in cdr_list:  #read CDR3s line by line
    ...
    if line in cdr3_dict:  #check if the cdr3 is unique
        ...
        cdr3_dict[line] = 1
    if not cdr3_count % 100000:
        print '%d of LCDR3 DNA sequences have been searched so far.' % cdr3_count
cdr_in.close()
stars()
print 'There are %d unique LCDR3 DNA sequences found' % (len(cdr3_dict))
...
stars()
print 'Frequency\t\t\tLCDR3 DNA Sequence\n'
for cdr, index in sorted(cdr3_dict.items(), key=itemgetter(1), reverse=True):  #order the CDR3s by abundance
    f_out.write('%s, %s\n' % (cdr, index))  #the csv file contains unique seq, count
    if cdr3_count < 20:
        print index, '\t\t\t', cdr
...
f_out.close()
stars()
print'The %s file contains the unique LCDR3 DNA sequences and their abundance in a csv format' % bins_dna_out
...
print 'The %s file has been created containing %i LCDR3 peptide sequences.' % (cdr_file_name, len(cdr_list))
print 'The %s file has been created containing %i LCDR3 DNA sequences.' % (cdr_dna_file_name, len(cdr_list))
print 'The %s file has been created containing %i unique LCDR3 peptide sequences.' % (bins_out, no_of_unique_cdr3)
print 'The %s file has been created containing %i unique LCDR3 DNA sequences.' % (bins_dna_out, no_of_unique_cdr3_dna)
stars()
