...
for DNA_seq in fasta_list: #Go through each line of the trimmed fasta file
  ...
  if re.match('^[ACGTN]', DNA_seq): #check if the line contains seq ID or DNA seq
    ...
    if match:
      cdr3_count += 1
      cdr_file_out.write(protein_translate(aa_dict, match.group())[2:-3] + '\n') #write the CDR3 output file
      cdr_dna_file_out.write(match.group()[2:-1] + '\n') #write the CDR3 DNA output file

    else:
      rev_match = re.search(cdr3_pattern, DNA_seq) #check if the seq contains a CDR3
      if rev_match:
        cdr3_count += 1
        cdr_file_out.write(protein_translate(aa_dict, rev_match.group())[2:-3] + '\n') #write the CDR3 output file
        cdr_dna_file_out.write(rev_match.group()[2:-1] + '\n') #write the CDR3 DNA output file
