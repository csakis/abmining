...
if len(sys.argv) < 2:
  cdr_in = raw_input('Please input the sample name (i.e. sd6.dna.cdr3): ')
else:
  cdr_in = sys.argv[1]
...
for line in f_in.readlines(): #read CDR3s line by line
  values = line.split(',')
  cdr3_dna = values[0][:-1] #remove \n character from lines
  if cdr3_dna in cdr3_dict: #check if the cdr3 is unique
    cdr3_dict[cdr3_dna] += 1
  else:
    cdr3_dict[cdr3_dna] = 1
