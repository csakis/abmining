...
the found DNA sequunces into peptide sequences and saves the list as .cdr file."""
import sys
import re
...
if len(sys.argv) < 2:
  ...
  fasta_in = sys.argv[1]
cdr_out = fasta_in + '.dna.cdr3'

cdr_file_name = fasta_in + 'dna.cdr3' #The file name that will contain the HCDR3 sequences.
...
for DNA_seq in fasta_list: #Go through each line of the trimmed fasta file
  ...
  if re.match('^[ACGTN]', DNA_seq): #check if the line contains seq ID or DNA seq
    ...
    if match:
      cdr3_count += 1
      line_out = '%s, %s\n' % (match.group()[2:-1], protein_translate(aa_dict, match.group())[2:-1])
      cdr_file_out.write(line_out) #write the CDR3 output file
    else:
      rev_match = re.search(cdr3_pattern, DNA_seq) #check if the seq contains a CDR3
      if rev_match:
        cdr3_count += 1
        line_out = '%s, %s\n' % (rev_match.group()[2:-1], protein_translate(aa_dict, rev_match.group())[2:-1])
        cdr_file_out.write(line_out) #write the CDR3 output file
