...
for barcode, seq_list in barcode_dict.items():
  ...
  print 'Now we are looking for LCDR3 sequences in the %s sample' % barcode
  seq_count = 0
  cdr3_count = 0
  ...
  for DNA_seq in fasta_list: #Go through each line of the trimmed fasta file
    ...
    if re.match('^[ACGTN]', DNA_seq): #check if the line contains seq ID or DNA seq
      ...
      if match:
        cdr3_count += 1
        cdr_file_out.write(protein_translate(aa_dict, match.group())[2:-1] + '\n') #write the LCDR3 output file
        cdr_dna_file_out.write(match.group()[2:-3] + '\n') #write the LCDR3 DNA output file
      else:
        ...
        if rev_match:
          ...
          cdr_dna_file_out.write(rev_match.group()[2:-3] + '\n') #write the LCDR3 DNA output file
      if (seq_count % 250000) == 0:
        print 'So far %d sequences have been checked.' % seq_count
        #End CDR3 search loop
  print '\n%d LCDR3s were found in the %s sample.' % (cdr3_count, barcode)
  ...
  print '\nNow we will find each unique LCDR3s and their abundance.'
  ...
  cdr_in.close()
  print 'There are %d unique LCDR3s found in the %s sample.' % ((len(cdr3_dict)), barcode)
  ...
  print '\nNow we will find each unique LCDR3 DNA sequences and their abundance.'
  ...
  for line in cdr_list: # read CDR3s line by line
    ...
    if not cdr3_count % 100000:
      print '%d of LCDR3s have been searched so far.' % cdr3_count
  cdr_dna_in.close()
  print 'There are %d unique LCDR3 DNAs found in the %s sample.' % ((len(cdr3_dict)), barcode)
...
for barcode_seq, barcode_name in barcodes:
  ...
  print 'The %s file has been created containing %i LCDR3 sequences.' % (cdr_file_name, cdr3_count)
  print 'The %s file has been created containing %i unique LCDR3 peptide sequences.' % (bins_out, unique_count)
  print 'The %s file has been created containing %i unique LCDR3 DNA sequences.' % (bins_dna_out, unique_dna_count)
  stars()
